...
@time df_spatial, gene_names = Baysor.load_df("$DATA_PATH/ISS_h_brain_03_PreliminaryCalls.csv"; 
...
```julia
t_df = @where(Baysor.get_segmentation_df(bm_data_merged), :x .> 25000, :x .< 30000, :y .> 30000, :y .< 35000) |> deepcopy;
...
CSV.write("../output/iss_human_1/cm.tsv", Baysor.convert_segmentation_df_to_cm(segmentation_df), delim="\t")
CSV.write("../output/iss_human_1/cell_stat.csv", cell_stat_df[:, out_stat_names]);
CSV.write("../output/iss_human_1/segmentation.csv", segmentation_df[:, names(segmentation_df) .!= :mol_id]);
```
