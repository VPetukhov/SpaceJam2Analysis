...
@time df_spatial, gene_names = Baysor.load_df("$DATA_PATH/iss_human_hca10_1_spots.csv"; 
...
```julia
Baysor.append_confidence!(cur_df; nn_id=25, border_quantiles=(0.2, 0.5));
...
```julia
Baysor.append_confidence!(df_spatial; nn_id=25, border_quantiles=(0.2, 0.5));
```
...
```julia
@time polygons = Baysor.boundary_polygons(bm_data, grid_step=5.0, bandwidth=10.0, dens_threshold=1e-7);
```
...
```julia
Baysor.plot_cell_borders_polygons(t_df, polygons; color=t_gene_colors)
```

```julia
out_stat_names = names(cell_stat_df)[.!in.(names(cell_stat_df), Ref([:has_center, :x_prior, :y_prior]))];

CSV.write("../output/iss_human_hca10_1/cm.tsv", Baysor.convert_segmentation_df_to_cm(segmentation_df), delim="\t")
CSV.write("../output/iss_human_hca10_1/cell_stat.csv", cell_stat_df[:, out_stat_names]);
CSV.write("../output/iss_human_hca10_1/segmentation.csv", segmentation_df[:, names(segmentation_df) .!= :mol_id]);
```
