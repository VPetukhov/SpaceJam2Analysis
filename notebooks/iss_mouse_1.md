...
GENE_COMPOSITION_NEIGHBORHOOD = 10;
...
@time df_spatial, gene_names = Baysor.load_df("$DATA_PATH/ISS_m_brain_03_PreliminaryCalls.csv"; 
...
```julia
cur_df = @where(df_spatial, :x .> 12000, :x .< 15000, :y .> 12000, :y .< 15000) |> deepcopy;
...
```julia
Baysor.append_confidence!(cur_df; nn_id=10, border_quantiles=(0.2, 0.75));
...
```julia
Baysor.append_confidence!(df_spatial; nn_id=10, border_quantiles=(0.2, 0.75));
```
...
```julia
@time bm_data_merged = Baysor.run_bmm_parallel!(bm_data_arr_res, 300; min_molecules_per_cell=MIN_MOLECULES_PER_CELL, 
#     split_period=100, n_expression_clusters=10, min_cluster_size=10, n_clustering_pcs=30, clustering_distance=Distances.CosineDist(),
...
```julia
t_df = @where(Baysor.get_segmentation_df(bm_data_merged), :x .> 15000, :x .< 20000, :y .> 15000, :y .< 20000) |> deepcopy;
size(t_df)
```
...
```julia
@time polygons = Baysor.boundary_polygons(t_df, t_df.cell, grid_step=20.0, 
...
CSV.write("../output/iss_mouse_1/cm.tsv", Baysor.convert_segmentation_df_to_cm(segmentation_df), delim="\t")
CSV.write("../output/iss_mouse_1/cell_stat.csv", cell_stat_df[:, out_stat_names]);
CSV.write("../output/iss_mouse_1/segmentation.csv", segmentation_df[:, names(segmentation_df) .!= :mol_id]);
```
