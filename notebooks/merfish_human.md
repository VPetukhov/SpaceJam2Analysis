...
```julia
@time df_spatial, gene_names = Baysor.load_df("$DATA_PATH/barcodes_287gene_0113_hMTG.csv"; 
...
```julia
# cur_df = @where(df_spatial, :x .> 10000, :x .< 13000, :y .> 7000, :y .< 10000) |> deepcopy;
# cur_df = @where(df_spatial, :global_z .< 1.5) |> deepcopy;
cur_df = df_spatial
size(cur_df)
```
...
```julia
Plots.histogram(bm_data.x.confidence)
```

```julia
Baysor.plot_cell_borders_polygons(cur_df, polygons; annotation=(bm_data.x.confidence .< 0.1) .* 1)
```
...
@time bm_data_arr = Baysor.initial_distribution_arr(df_spatial; scale=SCALE, min_molecules_per_cell=MIN_MOLECULES_PER_CELL,
    new_component_weight=NEW_COMPONENT_WEIGHT, n_frames=10, scale_std="40%");
```
...
```julia
t_df = @where(Baysor.get_segmentation_df(bm_data_merged), :x .> 5000, :x .< 10000, :y .> 10000, :y .< 15000) |> deepcopy;
...
CSV.write("../output/iss_mouse_2/cm.tsv", Baysor.convert_segmentation_df_to_cm(segmentation_df), delim="\t")
CSV.write("../output/iss_mouse_2/cell_stat.csv", cell_stat_df[:, out_stat_names]);
CSV.write("../output/iss_mouse_2/segmentation.csv", segmentation_df[:, names(segmentation_df) .!= :mol_id]);
```
