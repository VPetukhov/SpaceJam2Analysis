...
@time df_spatial, gene_names = Baysor.load_df("$DATA_PATH/Allen_smFISH_MCT_CZI_Panel_0_spot_table.csv"; 
...
```julia
cur_df = @where(df_spatial, :x .> -1500, :x .< -1000, :y .> -1500, :y .< -1000, :z .< 3) |> deepcopy;
size(cur_df)
```

```julia
cur_df = @where(df_spatial, :x .> -1500, :x .< -1000, :y .> -1500, :y .< -1000, :z .> 7) |> deepcopy;
size(cur_df)
```

```julia
cur_df = @where(df_spatial, :x .> -1500, :x .< -1000, :y .> -1500, :y .< -1000) |> deepcopy;
size(cur_df)
```
...
@time bm_data_arr = Baysor.initial_distribution_arr(df_spatial; min_molecules_per_cell=MIN_MOLECULES_PER_CELL, scale=SCALE,
    new_component_weight=NEW_COMPONENT_WEIGHT, n_frames=30, scale_std="50%");
```
...
@time t_polygons = Baysor.boundary_polygons(t_df, t_df.cell, grid_step=1.0, 
    dens_threshold=1e-3, bandwidth=2.0, min_molecules_per_cell=5);
```
...
CSV.write("../output/osm_fish_ex_0/cm.tsv", cm, delim="\t")
CSV.write("../output/osm_fish_ex_0/cell_stat.csv", cell_stat_df[:, out_stat_names]);
CSV.write("../output/osm_fish_ex_0/segmentation.csv", segmentation_df[:, names(segmentation_df) .!= :mol_id]);
```
